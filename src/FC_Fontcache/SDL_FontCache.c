...
static FC_GlyphData* FC_MapInsert(FC_Map* map, Uint32 codepoint, FC_GlyphData glyph)
{
    ...
    Uint32 index = codepoint % map->num_buckets;
...
static FC_GlyphData* FC_MapFind(FC_Map* map, Uint32 codepoint)
{
    ...
    for (FC_MapNode* node = map->buckets[index]; node != NULL; node = node->next)
    {
...
int U8_strinsert(char* string, int position, const char* source, int max_bytes)
{
    ...
    const int add_len = strlen(source);
...
void U8_strdel(char* string, int position)
{
    ...
    while (*string != '\0')
    {
        if (position == 0)
        {
	        const int chars_to_erase = U8_charsize(string);
	        const int remaining_bytes = strlen(string) + 1;
...
{
	float x = FC_MIN(A.x, B.x);
    float y = FC_MIN(A.y, B.y);
    float x2 = FC_MAX(A.x + A.w, B.x + B.w);
    float y2 = FC_MAX(A.y + A.h, B.y + B.h);
    {
...
void FC_GetUTF8FromCodepoint(char* result, Uint32 codepoint)
{
    ...
    char a = (codepoint >> 24) & 0xFF;
    char b = (codepoint >> 16) & 0xFF;
    char c = (codepoint >> 8) & 0xFF;
    char d = codepoint & 0xFF;
...
static FC_GlyphData* FC_PackGlyphData(FC_Font* font, Uint32 codepoint, Uint16 width, Uint16 maxWidth, Uint16 maxHeight)
{
    ...
    const Uint16 height = font->height + FC_CACHE_PADDING;
...
Uint8 FC_SetGlyphCacheLevel(FC_Font* font, int cache_level, FC_Image* cache_texture)
{
    ...
    if (cache_level == font->glyph_cache_count)
    {
        ...
        {
	        FC_Image** new_cache = (FC_Image**)malloc(font->glyph_cache_count * sizeof(FC_Image*));
            for (int i = 0; i < font->glyph_cache_size; ++i)
...
{
	FC_Font* font = (FC_Font*)malloc(sizeof(FC_Font));
...
#endif
{
...

    {
	    ...
	    const unsigned int w = font->height * 12;
	    const unsigned int h = font->height * 12;
        ...
        for (; *source_string != '\0'; source_string = U8_next(source_string))
        {
            ...
            if (!U8_charcpy(buff, source_string, 5))
                continue;
            SDL_Surface* glyph_surf = TTF_RenderUTF8_Blended(ttf, buff, white);
            if (glyph_surf == NULL)
                continue;

            // Try packing.  If it fails, create a new surface for the next cache level.
            Uint8 packed = (FC_PackGlyphData(font, FC_GetCodepointFromUTF8(&buff_ptr, 0), glyph_surf->w,
...
#endif
{
    ...
    Uint8 outline = (style & TTF_STYLE_OUTLINE);
    if (outline)
    {
...
unsigned int FC_GetNumCodepoints(FC_Font* font)
{
    ...
    for (int i = 0; i < glyphs->num_buckets; ++i)
    {
	    for (FC_MapNode* node = glyphs->buckets[i]; node != NULL; node = node->next)
        {
            result++;
        }
    }
...
{
	const FC_GlyphData* e = FC_MapFind(font->glyphs, codepoint);
    if (e == NULL)
    {
        char buff[5];
        int w, h;
        ...
        FC_Image* cache_image = FC_GetGlyphCacheLevel(font, font->last_glyph.cache_level);
        if (cache_image == NULL)
        {
...
static FC_Rect FC_RenderLeft(FC_Font* font, FC_Target* dest, float x, float y, FC_Scale scale, const char* text)
{
    ...
    float destH = font->height * scale.y;
    float destLineSpacing = font->lineSpacing * scale.y;
    float destLetterSpacing = font->letterSpacing * scale.x;

    if (c == NULL || font->glyph_cache_count == 0 || dest == NULL)
        return dirtyRect;

    const int newlineX = x;
    ...
    for (; *c != '\0'; c++)
    {
        ...
        Uint32 codepoint = FC_GetCodepointFromUTF8(&c, 1);  // Increments 'c' to skip the extra UTF-8 bytes
...
{
	const int num_levels = FC_GetNumCacheLevels(font);
    for (int i = 0; i < num_levels; ++i)
    {
	    FC_Image* img = FC_GetGlyphCacheLevel(font, i);
...
static FC_StringList* FC_GetBufferFitToColumn(FC_Font* font, int width, FC_Scale scale, Uint8 keep_newlines)
{
    ...
    for (FC_StringList* iter = ls; iter != NULL; iter = iter->next)
    {
        ...
        {
            FC_StringList * word_iter, * spaces, * spaces_iter;

            FC_StringList* words = FC_ExplodeBreakingSpace(line, &spaces);
...
static void FC_DrawColumnFromBuffer(FC_Font* font, FC_Target* dest, FC_Rect box, int* total_height, FC_Scale scale, FC_AlignEnum align)
{
    ...
    FC_StringList* ls = FC_GetBufferFitToColumn(font, box.w, scale, 0);
    for (FC_StringList* iter = ls; iter != NULL; iter = iter->next)
    {
...
Uint16 FC_GetHeight(FC_Font* font, const char* formatted_text, ...)
{
    ...
    for (const char* c = fc_buffer; *c != '\0'; c++)
    {
        if (*c == '\n')
            numLines++;
    }
...
Uint16 FC_GetWidth(FC_Font* font, const char* formatted_text, ...)
{
    ...
    for (c = fc_buffer; *c != '\0'; c++)
    {
        ...
        FC_GlyphData glyph;
        const Uint32 codepoint = FC_GetCodepointFromUTF8(&c, 1);
...
FC_Rect FC_GetCharacterOffset(FC_Font* font, Uint16 position_index, int column_width, const char* formatted_text, ...)
{
    ...
    FC_StringList* ls = FC_GetBufferFitToColumn(font, column_width, FC_MakeScale(1, 1), 1);
    for (FC_StringList* iter = ls; iter != NULL;)
    {
	    int i = 0;
        FC_StringList* next_iter = iter->next;

        ++num_lines;
        for (char* line = iter->value; line != NULL && *line != '\0'; line = (char*)U8_next(line))
        {
            ++i;
            --position_index;
            if (position_index == 0)
...
Uint16 FC_GetColumnHeight(FC_Font* font, Uint16 width, const char* formatted_text, ...)
{
    ...
    FC_StringList* ls = FC_GetBufferFitToColumn(font, width, FC_MakeScale(1, 1), 0);
    for (FC_StringList* iter = ls; iter != NULL; iter = iter->next)
    {
        y += FC_GetLineHeight(font);
    }
...
int FC_GetAscent(FC_Font* font, const char* formatted_text, ...)
{
    ...
    while (*c != '\0')
    {
        ...
        if (codepoint != 0)
        {
            int ascent = FC_GetAscentFromCodepoint(font, codepoint);
            if (ascent > max)
                max = ascent;
        }
        ++c;
...
int FC_GetDescent(FC_Font* font, const char* formatted_text, ...)
{
    ...
    while (*c != '\0')
    {
        ...
        if (codepoint != 0)
        {
            int descent = FC_GetDescentFromCodepoint(font, codepoint);
            if (descent > max)
                max = descent;
        }
        ++c;
...
{
	Uint8 done = 0;
    const int height = FC_GetLineHeight(font);
    Uint16 position = 0;
    int current_x = 0;
    int current_y = 0;
...
int FC_GetWrappedText(FC_Font* font, char* result, int max_result_size, Uint16 width, const char* formatted_text, ...)
{
    ...
    for (FC_StringList* iter = ls; iter != NULL && size_remaining > 0; iter = iter->next)
    {
        // Copy as much of this line as we can
        const int len = strlen(iter->value);
        const int num_bytes = FC_MIN(len, size_remaining);
